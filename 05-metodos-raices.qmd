...
![La cota global contiene todas las raíces; la malla y los cambios de signo producen intervalos de búsqueda más pequeños.](figures/acotacion-separacion-raices.png){#fig-acotacion-separacion width=90%}
...
El cambio de signo prueba existencia y la derivada positiva prueba unicidad en $[1,2]$.
...
coef_p_menos_x <- function(coef) {
  grado <- length(normalizar_coef(coef)) - 1L
  coef * (-1) ^ seq(grado, 0L)
}
...
biseccion <- function(coef, a, b, tol = 1e-8, max_iter = 100L) {
  ...
  if (fa == 0) return(data.frame(iter = 0L, a = a, b = a, x = a, fx = 0, error = 0))
  if (fb == 0) return(data.frame(iter = 0L, a = b, b = b, x = b, fx = 0, error = 0))
...
![Las primeras bisecciones reducen sistemáticamente el intervalo que encierra la raíz.](figures/biseccion-raiz.png){#fig-biseccion width=86%}
...
![La secante usa una cuerda definida por dos puntos; Newton usa una tangente y requiere la derivada.](figures/secante-newton.png){#fig-secante-newton width=92%}
...
[Descargar el cuaderno](notebooks/capitulo-05-metodos-raices-R.ipynb){.btn}
:::
